...
//=============================================================================
/**
 * @file   modTM_VC_Demux_Interface.cpp
 * @author Tad Kollar  
 *
 * $Id: modTM_VC_Demux_Interface.cpp 2375 2013-09-27 18:10:39Z tkollar $
 * Copyright (c) 2012.
...
//=============================================================================
/**
 * @class modTM_VC_Demux_Interface
 * @author Tad Kollar  
 * @brief Provide an XML-RPC interface to the TM Virtual Channel Demultiplexing module.
 */
//=============================================================================
class modTM_VC_Demux_Interface: public nasaCE::TrafficHandler_Interface<modTM_VC_Demux> {
public:
	modTM_VC_Demux_Interface(xmlrpc_c::registryPtr& rpcRegistry):
		nasaCE::TrafficHandler_Interface<modTM_VC_Demux>(rpcRegistry, "modTM_VC_Demux") {
		ACE_TRACE("modTM_VC_Demux_Interface::modTM_VC_Demux_Interface");

		register_methods(rpcRegistry);
	}

	void connect_output(xmlrpc_c::paramList const& paramList, xmlrpc_c::value* retvalP) {
		ACE_TRACE("modTM_VC_Demux_Interface::connect_output");
		...
		modTM_VC_Demux* handler = dynamic_cast<modTM_VC_Demux*>(channel->find(handlerName));
	...
	void connect_primary_output(xmlrpc_c::paramList const& paramList, xmlrpc_c::value* retvalP) {
		ACE_TRACE("modTM_VC_Demux_Interface::connect_primary_output");
	...
	void get_output_links(xmlrpc_c::paramList const& paramList, xmlrpc_c::value* retvalP) {
		ACE_TRACE("modTM_VC_Demux_Interface::get_output_links");

		modTM_VC_Demux* handler = dynamic_cast<modTM_VC_Demux*>(find_handler(paramList));
...
protected:
	void get_counters_(xmlrpc_c::paramList const& paramList, modTM_VC_Demux* handler,
		xstruct& counters) {
		ACE_TRACE("modTM_VC_Demux_Interface::get_counters_");

		nasaCE::TrafficHandler_Interface<modTM_VC_Demux>::get_counters_(paramList, handler, counters);

		GET_TM_PHYSICAL_CHANNEL_COUNTERS();
		GET_TM_MASTER_CHANNEL_COUNTERS();
	}

	void get_settings_(xmlrpc_c::paramList const& paramList, modTM_VC_Demux* handler,
		xstruct& settings) {
		ACE_TRACE("modTM_VC_Demux_Interface::get_settings_");

		nasaCE::TrafficHandler_Interface<modTM_VC_Demux>::get_settings_(paramList, handler, settings);
...
modTM_VC_Demux_Interface* modTM_VC_Demux_InterfaceP;

TEMPLATE_METHOD_CLASSES(modTM_VC_Demux_Interface, modTM_VC_Demux_InterfaceP);
TM_PHYSICAL_CHANNEL_GENERATE_METHODS(modTM_VC_Demux_InterfaceP);
TM_MASTER_CHANNEL_GENERATE_METHODS(modTM_VC_Demux_InterfaceP);

void modTM_VC_Demux_Interface::register_methods(xmlrpc_c::registryPtr& rpcRegistry) {
	ACE_TRACE("modTM_VC_Demux_Interface::register_methods");

	TEMPLATE_REGISTER_METHODS(modTM_VC_Demux, modTM_VC_Demux_Interface);
	TM_PHYSICAL_CHANNEL_REGISTER_METHODS(modTM_VC_Demux);
	TM_MASTER_CHANNEL_REGISTER_METHODS(modTM_VC_Demux);
}
