...
<modinfo xmlns="http://channel-emulator.grc.nasa.gov/module-properties">
  <name>modAOS_MC_Demux</name>
  <title>Master Channel De-multiplexing Function</title>
  <cat>AOS</cat>
  <depends>
  </depends>
  <desc>This module accepts AOS Transfer Frame wrappers on its primary input, separates them based on their Master Channel Identifier (MCID), and sends them out an associated primary output. There can be as many primary outputs as there are possible master channels in one physical channel, and each is associated with a single MCID.</desc>
  <config>
    <ref>
