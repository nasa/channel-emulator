...
//=============================================================================
/**
 * @file   AOS_Demux_Base.cpp
 * @author Tad Kollar  
 *
 * $Id: AOS_Demux_Base.cpp 2373 2013-09-25 18:28:26Z tkollar $
 * Copyright (c) 2009.
...
AOS_Demux_Base::AOS_Demux_Base(const std::string& newName, const std::string& newChannelName,
...
AOS_Demux_Base::~AOS_Demux_Base() {
	MOD_DEBUG("Running ~AOS_Demux_Base().");
}

void AOS_Demux_Base::withdraw(const bool doStopTraffic /* = true */) {
...
bool AOS_Demux_Base::isCircularConnection(BaseTrafficHandler* testTarget) {
...
void AOS_Demux_Base::connectOutput(BaseTrafficHandler* target,
	...
	if (sourceOutputRank == HandlerLink::PrimaryOutput)
		throw nd_error("AOS_Demux_Base::connectOutput() without channel identifier used for PrimaryOutput.");
...
void AOS_Demux_Base::connectPrimaryOutput(const ACE_UINT8 identifier,
...
void AOS_Demux_Base::disconnectOutput(const HandlerLink::OutputRank sourceOutputRank
...
HandlerLinkMap::iterator AOS_Demux_Base::findLink_(HandlerLink* link) {
	HandlerLinkMap::iterator pos;
...
void AOS_Demux_Base::getIdentifiers(std::list<ACE_UINT8>& idList) {
	idList.clear();
...
void AOS_Demux_Base::deleteLink_(const LinkType linkType, HandlerLink* oldLink) {
...
void AOS_Demux_Base::setLink_(const LinkType linkType, HandlerLink* newLink) {
...
void AOS_Demux_Base::unsetLink_(const LinkType linkType, HandlerLink* oldLink) {
	if ( linkType == PrimaryOutputLink ) {
		HandlerLinkMap::iterator pos = findLink_(oldLink);
