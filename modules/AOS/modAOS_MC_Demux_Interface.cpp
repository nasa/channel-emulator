...
//=============================================================================
/**
 * @file   modAOS_MC_Demux_Interface.cpp
 * @author Tad Kollar  
 *
 * $Id: modAOS_MC_Demux_Interface.cpp 2375 2013-09-27 18:10:39Z tkollar $
 * Copyright (c) 2009.
...
//=============================================================================
/**
 * @class modAOS_MC_Demux_Interface
 * @author Tad Kollar  
 * @brief Provide an XML-RPC interface to the Master Channel Demultiplexing module.
 */
//=============================================================================
class modAOS_MC_Demux_Interface: public nasaCE::TrafficHandler_Interface<modAOS_MC_Demux> {
public:
	modAOS_MC_Demux_Interface(xmlrpc_c::registryPtr& rpcRegistry):
		nasaCE::TrafficHandler_Interface<modAOS_MC_Demux>(rpcRegistry, "modAOS_MC_Demux") {
		ACE_TRACE("modAOS_MC_Demux_Interface::modAOS_MC_Demux_Interface");

		register_methods(rpcRegistry);
	}

	void connect_output(xmlrpc_c::paramList const& paramList, xmlrpc_c::value* retvalP) {
		ACE_TRACE("modAOS_MC_Demux_Interface::connect_output");
		std::string channelName(paramList.getString(0));
		std::string handlerName(paramList.getString(1));
		ACE_UINT8 SCID = paramList.getInt(2);
		...
		modAOS_MC_Demux* handler = dynamic_cast<modAOS_MC_Demux*>(channel->find(handlerName));
		BaseTrafficHandler* targetHandler = channel->find(targetName);

		if ( outputRank == HandlerLink::PrimaryOutput )
			handler->connectPrimaryOutput(SCID, targetHandler, inputRank);
		else
	...
	void connect_primary_output(xmlrpc_c::paramList const& paramList, xmlrpc_c::value* retvalP) {
		ACE_TRACE("modAOS_MC_Demux_Interface::connect_primary_output");
	...
	void get_output_links(xmlrpc_c::paramList const& paramList, xmlrpc_c::value* retvalP) {
		ACE_TRACE("modAOS_MC_Demux_Interface::get_output_links");

		modAOS_MC_Demux* handler = dynamic_cast<modAOS_MC_Demux*>(find_handler(paramList));
...
protected:
	...
	void get_counters_(xmlrpc_c::paramList const& paramList, modAOS_MC_Demux* handler,
		xstruct& counters) {
		ACE_TRACE("modAOS_MC_Demux_Interface::get_counters_");

		nasaCE::TrafficHandler_Interface<modAOS_MC_Demux>::get_counters_(paramList, handler, counters);

		GET_AOS_PHYSICAL_CHANNEL_COUNTERS();
	}

	void get_settings_(xmlrpc_c::paramList const& paramList, modAOS_MC_Demux* handler,
		xstruct& settings) {
		ACE_TRACE("modAOS_MC_Demux_Interface::get_settings_");

		nasaCE::TrafficHandler_Interface<modAOS_MC_Demux>::get_settings_(paramList, handler, settings);

		GET_AOS_PHYSICAL_CHANNEL_SETTINGS();
	}
};

modAOS_MC_Demux_Interface* modAOS_MC_Demux_InterfaceP;

TEMPLATE_METHOD_CLASSES(modAOS_MC_Demux_Interface, modAOS_MC_Demux_InterfaceP);
AOS_PHYSICAL_CHANNEL_GENERATE_METHODS(modAOS_MC_Demux_InterfaceP);

void modAOS_MC_Demux_Interface::register_methods(xmlrpc_c::registryPtr& rpcRegistry) {
	ACE_TRACE("modAOS_MC_Demux_Interface::register_methods");

	TEMPLATE_REGISTER_METHODS(modAOS_MC_Demux, modAOS_MC_Demux_Interface);
	AOS_PHYSICAL_CHANNEL_REGISTER_METHODS(modAOS_MC_Demux);
}
